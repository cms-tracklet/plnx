...
EN_RESULT Eeprom_ReadBasicModuleInfo()
{
	switch (g_EepromDeviceType)
			...
			case EEepromDevice_AtmelAtsha204a:
			{
				...
				char ethaddr[30];
				sprintf(ethaddr, "ethaddr=%02x:%02x:%02x:%02x:%02x:%02x\n", fullMacAddressBuffer[0], fullMacAddressBuffer[1], fullMacAddressBuffer[2], fullMacAddressBuffer[3], fullMacAddressBuffer[4], fullMacAddressBuffer[5]);
				// Copy string to memory
				for(int i=0; i< strlen(ethaddr)+1; i++) {
					volatile u8 *LocalAddr = (volatile u8 *)(0xFFFFFC00+i);
					*LocalAddr = ethaddr[i];
				}
